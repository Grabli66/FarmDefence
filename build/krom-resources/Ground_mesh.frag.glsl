...
float tex_noise_f(vec3 x)
{
    ...
    vec2 rg = texture(snoise256, (uv + vec2(0.5)) / vec2(256.0)).yx;
    return mix(rg.x, rg.y, f.z);
}

float tex_noise(inout vec3 p)
{
    p *= 1.25;
    float f = 0.5 * tex_noise_f(p);
...
float PCFCube(samplerCube shadowMapCube_1, vec3 lp, inout vec3 ml, float bias, vec2 lightProj_1, vec3 n)
{
    ...
    float _490 = lpToDepth(param, lightProj_1);
    float compare = _490 - (bias * 0.4000000059604644775390625);
...
void main()
{
    ...
    float _722 = tex_noise(param);
    vec3 param_1 = (bposition * 20.0) + vec3(0.3300000131130218505859375);
    float _729 = tex_noise(param_1);
    vec3 param_2 = (bposition * 20.0) + vec3(0.660000026226043701171875);
    float _736 = tex_noise(param_2);
    vec3 NoiseTexture_Color_res = vec3(_722, _729, _736);
    vec3 basecol = vec3(0.0050248322077095508575439453125, 0.0324445404112339019775390625, 0.00257560913451015949249267578125) + (NoiseTexture_Color_res * 0.0130000002682209014892578125);
    float roughness = 0.0;
    float metallic = 0.0;
    float occlusion = 1.0;
    float specular = 0.0;
    ...
    if (receiveShadow)
    {
        ...
        else
        {
            ...
            {
                vec3 param_3 = -l;
                float _843 = PCFCube(shadowMapCube, lp, param_3, shadowsBias, lightProj, n);
                visibility *= _843;
            }
        }
    }
    ...
    vec3 _926 = tonemapFilmic(fragColor.xyz);
