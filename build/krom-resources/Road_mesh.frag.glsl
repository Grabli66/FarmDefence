...
vec3 tex_brick(inout vec3 p, vec3 c1, vec3 c2, vec3 c3)
{
    p /= vec3(0.449999988079071044921875, 0.24500000476837158203125, 0.24500000476837158203125);
    if (fract(p.y * 0.5) > 0.5)
    {
        p.x += 0.5;
    }
    p = fract(p);
    vec3 b = step(p, vec3(0.949999988079071044921875, 0.89999997615814208984375, 0.89999997615814208984375));
    return mix(c3, c1, vec3((b.x * b.y) * b.z));
}
...
float PCFCube(samplerCube shadowMapCube_1, vec3 lp, inout vec3 ml, float bias, vec2 lightProj_1, vec3 n)
{
    ...
    float _489 = lpToDepth(param, lightProj_1);
    float compare = _489 - (bias * 0.4000000059604644775390625);
...
void main()
{
    ...
    vec3 param = bposition * 20.0;
    vec3 _676 = tex_brick(param, vec3(0.800000011920928955078125), vec3(0.20000000298023223876953125), vec3(0.0));
    vec3 BrickTexture_Color_res = _676;
    vec3 basecol = vec3(0.0425266139209270477294921875, 0.0431161262094974517822265625, 0.00229728850536048412322998046875) + (BrickTexture_Color_res * 0.00999999977648258209228515625);
    float roughness = 0.0;
    float metallic = 0.0;
    float occlusion = 1.0;
    float specular = 0.0;
    ...
    if (receiveShadow)
    {
        ...
        else
        {
            ...
            {
                vec3 param_1 = -l;
                float _781 = PCFCube(shadowMapCube, lp, param_1, shadowsBias, lightProj, n);
                visibility *= _781;
            }
        }
    }
    ...
    vec3 _864 = tonemapFilmic(fragColor.xyz);
    fragColor = vec4(_864.x, _864.y, _864.z, fragColor.w);
}
